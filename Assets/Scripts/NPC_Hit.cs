...
public class EnemyMoveHit : MonoBehaviour {
    ...
    void Start()
    {
        ...
        if (GameObject.FindGameObjectWithTag ("NPC") != null) {
                     target2 = GameObject.FindGameObjectWithTag ("NPC").GetComponent<Transform> ();
              }
       ...
       void Update () {
              ...
              float DistToNPC = Vector3.Distance(transform.position, target2.position);
        ...
        else if ((target2 != null) && (DistToNPC <= attackRange))
        {
            GetComponent<NPC_PatrolSequencePoints>().enabled = false;
            ...
            if (target2.position.x > gameObject.transform.position.x)
            {
                gameObject.transform.localScale = new Vector2(scaleX, gameObject.transform.localScale.y);
            }
            else
